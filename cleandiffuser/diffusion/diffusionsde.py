...
class DiscreteDiffusionSDE(BaseDiffusionSDE):
    ...
    def sample(
        ...
        if isinstance(warm_start_reference, torch.Tensor):
            ...
            print(warm_start_forward_level, diffusion_steps)
            fwd_alpha, fwd_sigma = self.alpha[diffusion_steps], self.sigma[diffusion_steps]
...
class ContinuousDiffusionSDE(BaseDiffusionSDE):
    ...
    def compute_rg(
        ...
        if not self.predict_noise:
            ...
            # alpha_t = at_least_ndim(self.alphas[t], xt.dim())
            # sigma_t = at_least_ndim(self.sigmas[t], xt.dim())
        ...
        for i in range(n_mc_samples):
            # diffusion_steps = int(forward_level * self.diffusion_steps)
            # fwd_alpha, fwd_sigma = self.alphas[diffusion_steps], self.sigmas[diffusion_steps]
            t_hat = torch.full((x0.shape[0],), 1.0 * forward_level, dtype=torch.long, device=self.device)
